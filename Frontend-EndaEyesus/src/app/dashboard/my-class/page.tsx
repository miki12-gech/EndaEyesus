...
export default function MyClassPage() {
  ...
  // Only allow service managers with a class
  ...
  const handleAssignClick = (member: any) => {
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="text-center mb-8">
          <motion.div
            ...
            className="inline-flex items-center gap-2 bg-[#C9A227]/10 backdrop-blur-sm rounded-full px-4 py-2 mb-4 border border-[#C9A227]/30"
          >
            <Users className="h-4 w-4 text-[#C9A227]" />
            <span className="text-sm font-medium text-[#7A1C1C] dark:text-[#D4AF37]">
              {className} · አባላት
            </span>
          </motion.div>
          <h1 className="text-3xl font-bold text-foreground">የክፍል አባላት</h1>
          <p className="text-muted-foreground mt-1">በክፍልዎ ውስጥ ያሉትን ሁሉንም አባላት ይመልከቱ እና ያስተዳድሩ</p>
        </div>
        ...
        <div className="relative max-w-md mb-8">
          ...
          <Input
            placeholder="ፈልግ በሙሉ ስም፣ ኢሜይል ወይም ዩኒቨርሲቲ መታወቂያ..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="pl-9 bg-white/80 dark:bg-[#1C1C1F]/80 backdrop-blur-sm border-[#C9A227]/40 focus:border-[#C9A227] transition-all"
          />
        </div>
          ...
          <div className="overflow-x-auto rounded-2xl border border-[#ddd8d0] dark:border-[#2a2a2d] bg-white/70 dark:bg-[#1C1C1F]/70 backdrop-blur-sm shadow-xl">
            <table className="min-w-full divide-y divide-[#ddd8d0] dark:divide-[#2a2a2d]">
              <thead className="bg-gradient-to-r from-[#7A1C1C]/5 to-[#C9A227]/5 dark:from-[#7A1C1C]/10 dark:to-[#D4AF37]/10">
                <tr>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-[#7A1C1C] dark:text-[#D4AF37]">አባል</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-[#7A1C1C] dark:text-[#D4AF37]">ኢሜይል</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-[#7A1C1C] dark:text-[#D4AF37]">መታወቂያ</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-[#7A1C1C] dark:text-[#D4AF37]">ዝርዝር</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-[#7A1C1C] dark:text-[#D4AF37]">ድርጊት</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#ddd8d0] dark:divide-[#2a2a2d]">
                {filteredMembers.map((member: any) => (
                  <Fragment key={member.id}>
                    <tr
                      className="group hover:bg-[#F8F5F0] dark:hover:bg-[#252529] transition-colors duration-200 cursor-pointer"
                      onClick={() => toggleExpand(member.id)}
                    >
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-3">
                          <Avatar className="h-10 w-10 border-2 border-[#C9A227]">
                            <AvatarFallback className="bg-gradient-to-br from-[#7A1C1C] to-[#C9A227] text-white font-bold">
                              {member.full_name_three_parts?.split(" ").map((n: string) => n[0]).join("").slice(0, 2).toUpperCase() || "??"}
                            </AvatarFallback>
                          </Avatar>
                          <span className="font-medium text-foreground">{member.full_name_three_parts}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <Button
                          variant="ghost"
                          size="sm"
                          className="gap-1 text-[#C9A227] hover:text-[#B8911A]"
                          onClick={(e) => { e.stopPropagation(); toggleExpand(member.id); }}
                        >
                          {expandedMemberId === member.id ? (
                            <><ChevronDown className="h-4 w-4" /> ዝጋ</>
                          ) : (
                            <><ChevronRight className="h-4 w-4" /> ዘርጋ</>
                          )}
                        </Button>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <tr className="bg-[#F8F5F0]/50 dark:bg-[#252529]/50">
                        <td colSpan={5} className="px-6 py-5">
                          ...
                          <div className="mt-4 text-center text-xs text-muted-foreground">
                            ✙ “እንደ ሰውነት አንድ ሆኖ ብዙ አካላት እንዳሉት፣ እንዲሁም ሁላችን በአንድ መንፈስ ወደ አንድ ሰውነት ተጠመቅን” (1ኛ ቆሮንቶስ 12:12)
                          </div>
                        </td>
                      </tr>
                    )}

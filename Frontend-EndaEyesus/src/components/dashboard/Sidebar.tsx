...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const canAccessMemberAffairs =
    (role === "SERVICE_MANAGER" &&
      user?.serviceClassName === "የአባልነት ጉዳይ ክፍል") ||
    isSecretariat;
  ...
  return (
    <aside
      className={`w-72 min-h-screen bg-white dark:bg-[#1C1C1F] flex flex-col fixed left-0 top-0 z-50 shadow-2xl border-r border-[#ddd8d0] dark:border-[#2a2a2d] transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}
    >
      {/* Logo */}
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto max-h-[calc(100vh-180px)]">
        {mainNavItems.map(({ href, label, icon: Icon }) => {
          ...
          return (
            ...
            >
              <Icon
                className={`h-5 w-5 transition-transform group-hover:scale-110 ${isActive ? "text-[#C9A227] dark:text-[#D4AF37]" : ""}`}
              />
              {label}
              {isActive && (
